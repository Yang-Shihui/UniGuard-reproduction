...
class Generator:
    ...
    def generate(self, prompt, image):
        ...
        # 修复：检查 output_ids 长度是否足够，避免索引越界
        if output_token_len >= input_token_len:
            n_diff_input_output = (input_ids != output_ids[:, :input_token_len]).sum().item()
            if n_diff_input_output > 0:
                print(f'[Warning] {n_diff_input_output} output_ids are not the same as the input_ids')
        else:
            print(f'[Warning] output_ids length ({output_token_len}) is shorter than input_ids length ({input_token_len}), skipping comparison')
        ...
        if output_token_len > input_token_len:
            ...
            # 如果输出长度不足，返回空字符串
            print(f'[Warning] No new tokens generated, output length ({output_token_len}) <= input length ({input_token_len})')
            outputs = ""
